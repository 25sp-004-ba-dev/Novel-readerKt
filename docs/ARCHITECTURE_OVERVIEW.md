...
- **Topological Safeguard**: The `onUrlSynced` receiver inside `WtrWebAppInterface` is heavily guarded. It always dynamically resolves the _currently active tab_ from the viewport (`viewModel.currentTab.value`) and checks if the WebView triggering the background event matches the active tab's unique ID:
